...
Code Review Agent
=================
A Google ADK agent that reviews code snippets for smells, security issues,
and complexity — using pure Python AST analysis (no external APIs needed).
...
def detect_code_smells(code: str, language: str = "python") -> dict:
    ...
    Checks for: long functions, excessive nesting, magic numbers,
    too many parameters, duplicate logic indicators, and missing docstrings.

    Args:
        code: The source code snippet to analyze.
        language: Programming language hint ("python", "javascript", etc.).
                  Full AST analysis only available for Python.

    Returns:
        dict with 'status' and 'smells' list (each with name, severity, detail)
        or 'error_message'.
    """
    if not code.strip():
    ...
    todo_hits = [f"line {i+1}" for i, l in enumerate(lines) if re.search(r'\bTODO\b|\bFIXME\b|\bHACK\b', l, re.I)]
    if todo_hits:
        smells.append({
...
def check_security_patterns(code: str) -> dict:
    ...
    Checks for: hardcoded credentials, SQL injection risk, command injection,
    use of eval/exec, insecure deserialization, and plaintext secret patterns.

    Args:
        code: The source code snippet to scan.

    Returns:
        dict with 'status' and 'issues' list (each with name, severity, line, detail)
        or 'error_message'.
    """
    if not code.strip():
        return {"status": "error", "error_message": "code cannot be empty."}
...
def calculate_complexity_metrics(code: str) -> dict:
    """Compute cyclomatic complexity and other metrics for a Python code snippet.

    Cyclomatic complexity = number of decision points + 1.
    Decision points: if, elif, for, while, except, with, assert, and/or in conditions.

    Args:
        code: Python source code snippet to measure.

    Returns:
        dict with 'status' and 'metrics' (complexity, line_count, function_count,
        class_count, complexity_rating) or 'error_message'.
    """
    if not code.strip():
        return {"status": "error", "error_message": "code cannot be empty."}
