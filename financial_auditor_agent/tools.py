"""
Financial Auditor — tool functions (no ADK dependency).
Imported by agent.py and directly by tests.
"""
...
def calculate_financial_ratios(
    ...
    Returns:
        dict with 'status' and 'ratios' (named ratio values + interpretations)
        or 'error_message'.
    """
    if revenue <= 0:
    ...
    ratios = {
        "gross_margin_pct": {
            "value": gross_margin,
            "interpretation": "Healthy (>40%)" if gross_margin > 40 else "Low (<20%)" if gross_margin < 20 else "Average",
        },
...
def detect_anomalies(
    ...
    Args:
        ...
        z_score_threshold: Flag transactions whose |z-score| exceeds this (default 2.5).

    Returns:
        dict with 'status', 'anomalies' list, and 'summary' stats, or 'error_message'.
    """
